...
function initCommon()
{
   if($GPGT::MPDebug) echo("\c3 ====> START-INIT /common/main.cs ->" SPC "initCommon()");
...
{
   if($GPGT::MPDebug) echo("\c3 ====> START-INIT /common/main.cs ->" SPC "initBaseClient()");
...
{
   if($GPGT::MPDebug) echo("\c3 ====> START-INIT /common/main.cs ->" SPC "initBaseServer()");
...
function parseArgs()
{
   if($GPGT::MPDebug) echo("\c3 ====> START-INIT /common/main.cs ->" SPC "parseArgs()");
   Parent::parseArgs();
...
function onStart()
{
   if($GPGT::MPDebug) echo("\c3 ====> START-INIT /common/main.cs ->" SPC "onStart()");
   Parent::onStart();
...
function onExit()
{
   if($GPGT::MPDebug) echo("\c3 ====> START-INIT /common/main.cs ->" SPC "onExit()");
...
if($GPGT::MPDebug) echo("\c3 ====> START-INIT /common/main.cs ->" SPC "activating package: Common");
...
function testchunk()
{
   ...
   new TextChunk()
   {
    ...
    saveChunkFile(%foo, "gpgt/test.chunk");
    
    %foo.delete();
    
    $foo = loadChunkFile("gpgt/test.chunk");
