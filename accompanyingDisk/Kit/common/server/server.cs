...
{
   if($GPGT::MPDebug) echo("\c4 ====> SEL-MODE common/server/server.cs: " SPC "portInit(" SPC %port SPC ")" );
...
{
   if($GPGT::MPDebug) echo("\c4 ====> SEL-MODE common/server.cs: " SPC "createServer(" SPC %serverType SPC "," SPC %mission SPC ")" );
...
{
   if($GPGT::MPDebug) echo("\c9 ====> PURGE common/server.cs: " SPC "destroyServer()" );
   ...
   if($GPGT::MPDebug) echo("\c9 ====> PURGE common/server.cs: " SPC "deleting datablocks" );
   deleteDataBlocks();
...
{
   if($GPGT::MPDebug) echo("\c5 ====> CONNECT common/server.cs: " SPC "onServerInfoQuery()" );
   ...
   // the ServerInfo::Status variable.
