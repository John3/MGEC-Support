...
{
   if($GPGT::MPDebug) echo("\c5 ====> LOAD-SERVER /common/server/missionLoad.cs ->:" SPC "loadMission(" SPC %missionName SPC "," SPC %isFirstMission SPC ")" );
...
{
   if($GPGT::MPDebug) echo("\c5 ====> LOAD-SERVER /common/server/missionLoad.cs ->:" SPC "loadMissionStage2()" );
...
function endMission()
{
   if($GPGT::MPDebug) echo("\c9 ====> PURGE /common/server/missionLoad.cs ->:" SPC "endMission()" );
...
{
   if($GPGT::MPDebug) echo("\c9 ====> PURGE /common/server/missionLoad.cs ->:" SPC "resetMission()" );
