...
function parseArgs()
{
   if($GPGT::MPDebug) echo("\c3 ====> START-INIT /gpgt/main.cs ->" SPC "parseArgs()");
   Parent::parseArgs();
...
function onStart()
{
   if($GPGT::MPDebug) echo("\c3 ====> START-INIT /gpgt/main.cs ->" SPC "onStart()");
   Parent::onStart();
   echo("\n--------- Initializing MOD: FPS Starter Kit ---------");

   exec("./HOWUtils/init.cs"); //EFM
...
function onExit()
{
   if($GPGT::MPDebug) echo("\c3 ====> START-INIT /gpgt/main.cs ->" SPC "onExit()");
...
if($GPGT::MPDebug) echo("\c3 ====> START-INIT /gpgt/main.cs ->" SPC "activating package: FpsStarterKit");
