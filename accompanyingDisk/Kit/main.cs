...
// GPGT - EFM mesage about why this is here

// Select our own logging level
setLogMode(6);
$logModeSpecified = false;

//GPGT
// \c3 - Dark blue  (Startup and Init)
// \c4 - Dark green (Mode Selection)
// \c5 - Dark Cyan  (Load Mission - Server)
// \c6 - Purple     (Connection)
// \c7 - Pink       (Load Mission - Client)
// \c8 - Orange     (Game Begin)
// \c9 - Brown      (Resource Purge)
$GPGT::MPDebug = false;
...
for ($i = 1; $i < $Game::argc ; $i++)
{
   ...
   {
      if($GPGT::MPDebug) echo("\c4 ====> SEL-MODE main.cs: " SPC "Set dedicated server mode." );
...
function onStart()
{
   if($GPGT::MPDebug) echo("\c3 ====> START-INIT /main.cs ->" SPC "onStart()");
   // Default startup function
}

function onExit()
{
   if($GPGT::MPDebug) echo("\c3 ====> START-INIT /main.cs ->" SPC "onExit()");
...
function parseArgs()
{
   if($GPGT::MPDebug) echo("\c3 ====> START-INIT /main.cs ->" SPC "parseArgs()");
...
if($GPGT::MPDebug) echo("\c3 ====> START-INIT /main.cs ->" SPC "--------- Loading MODS ---------");
